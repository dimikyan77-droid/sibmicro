...
${urls.map(escapeXml).join("\n")}
</urlset>`;
    // NOTE: we did NOT escape the URLs themselves — escapeXml above is wrong; rebuild without it
    const xmlClean = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
${urls.join("\n")}
</urlset>`;

    return new Response(xmlClean, {
      headers: {
        ...corsHeaders,
