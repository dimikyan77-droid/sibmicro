    ...
    <Layout>
      ...
      <div className="bg-muted border-b border-border">
        <div className="container py-3 text-xs text-muted-foreground flex items-center gap-1.5">
          ...
          <span>/</span>
          <span className="text-foreground font-medium">{t("header.manufacturers")}</span>
        </div>
      </div>
      <div className="container py-8">
        <h1 className="text-2xl font-bold text-foreground mb-6">{t("mfg.title")}</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {manufacturers.map((m) => (
            ...
            >
              ...
              <div className="text-xs text-accent font-semibold mt-3">{m.productCount.toLocaleString()} {t("mfg.products")}</div>
