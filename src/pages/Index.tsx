    ...
    <Layout>
        ...
        <div className="relative container py-16 md:py-24 text-primary-foreground">
          <div className="max-w-2xl">
            ...
            <p className="text-lg opacity-90 mb-8 leading-relaxed">
              Over 1,000,000 components in stock. Parametric search, datasheets, competitive pricing. Trusted by R&D teams and OEMs worldwide.
            </p>
            <form onSubmit={handleSearch} className="flex gap-2 max-w-xl">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search part number, keyword, manufacturer..."
                  value={searchQuery}
      ...
      <section className="bg-muted border-b border-border">
        <div className="container py-4 flex flex-wrap justify-center gap-8 md:gap-16 text-center">
          {[
            { icon: <Shield className="h-5 w-5 text-accent" />, label: "1M+ Parts", sub: "In Stock" },
            { icon: <Truck className="h-5 w-5 text-accent" />, label: "Same Day", sub: "Shipping" },
            { icon: <Headphones className="h-5 w-5 text-accent" />, label: "Expert", sub: "Support" },
          ].map((s) => (
            ...
            >
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  {cat.subcategories.reduce((acc, s) => acc + s.count, 0).toLocaleString()} parts
                </div>
              </div>
            </Link>
          ))}
      ...
      <section className="bg-muted/50">
        <div className="container py-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-foreground">Featured Components</h2>
              ...
              >
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                  ...
                  <span className={`chip ${p.stock > 0 ? "chip-success" : "chip-warning"}`}>
                    {p.stock > 0 ? `${p.stock.toLocaleString()} in stock` : "Lead time"}
                  </span>
                </div>
              </Link>
            ))}
      ...
      {/* Manufacturers */}
      <section className="container py-12">
        <h2 className="text-2xl font-bold text-foreground mb-6">Authorized Manufacturers</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {manufacturers.map((m) => (
            <Link
              key={m.slug}
            ...
            >
              ...
              <div className="text-xs text-muted-foreground mt-1">{m.productCount.toLocaleString()} parts</div>
            </Link>
          ))}
        </div>
      </section>
